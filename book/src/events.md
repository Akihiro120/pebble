...
Resources are good for "the current state of X" (see [Resources](./resources.md)). Events are for "something happened" — damage was dealt, a file finished loading, a button was clicked. `Events<T>` is a double-buffered queue: an event sent during tick `N` stays visible to every reader for the rest of `N` and all of `N + 1`, then is dropped.
...
Exactly like `Option<Res<T>>` (see [Resources](./resources.md#a-genuinely-optional-resource--optionrest)): use these when a system should just skip its event-related work if `T` hasn't been registered yet, instead of hard-panicking:
...
Events whose payload arrives from a background task — a downloaded file, a GPU readback — use a different constructor, `add_async_event`, covered in [Async Systems and Background Tasks](./async-and-background-tasks.md).
